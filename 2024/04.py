...
matches = lambda map, kern : torch.isclose(torch.nn.functional.conv2d((map.view(*[1] * max(0, 4 - len(map.shape)), *map.shape) if max(0, 4 - len(map.shape)) else map), (kern := kern.view(*[1] * max(0, 4 - len(kern.shape)), *kern.shape) if max(0, 4 - len(kern.shape)) else kern), padding="same"), (kern ** 2).sum(2, keepdim=True).sum(3, keepdim=True).squeeze(1).unsqueeze(0))

def part1(t_map : torch.Tensor) -> int:
    return matches(torch.nn.functional.pad(t_map, [5] * 4), torch.stack([(kern_flat := torch.zeros([5, 5]).index_put_((torch.zeros(4).long(), torch.arange(4)), (PATTERN := torch.tensor([CMAP[c] for c in "XMAS"])))), kern_flat.flip(1), kern_flat.T, kern_flat.flip(1).T, (kern_diag := torch.zeros([5, 5]).index_put_((torch.arange(4), torch.arange(4)), PATTERN)), kern_diag.flip(1), kern_diag.flip(0), kern_diag.flip(0).flip(1)]).unsqueeze(1)).squeeze(0).sum(1).sum(1).sum().item()

# print(part1(parse_input("2024/inputs/04.test")))
print(part1(parse_input("2024/inputs/04.input")))
...
# print(part2(parse_input("2024/inputs/04.test")))
print(part2(parse_input("2024/inputs/04.input")))