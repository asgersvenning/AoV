...
part1 : Callable[[int, int], int] = lambda left, right : sum(abs(l - r) for l, r in zip(sorted(left), sorted(right)))

# print(part1(*parse_input("test")))
print(part1(*parse_input("input")))

part2 : Callable[[int, int], int] = lambda left, right : sum(n * freq.get(n, 0) for n in left) if ([freq.update({n : freq.get(n, 0) + 1}) for n in right] if not (freq := {}) else []) else 0
